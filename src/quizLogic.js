...
function normalizeQuestion(raw, index = 0) {
  const type = legacyTypeToModern(raw.type || raw['question-type'] || raw.questionType);
  const rawOptions = Array.isArray(raw.options) ? raw.options : [];
  const options = type === 'text'
    ? []
    : rawOptions.map((opt) => {
        if (opt && typeof opt === 'object') {
          return { id: opt.id || makeId('opt'), text: normalizeAnswerText(opt.text ?? opt.label ?? opt.value) };
        }
  ...
  if (type === 'text') {
    correctText = normalizeAnswerText(raw.correctText ?? raw.answer ?? raw.correctAnswer ?? '');
    ...
    const rawAnswer = raw.answer ?? raw.correctAnswer ?? (type === 'multi' ? [] : '');
    const answerTexts = Array.isArray(rawAnswer) ? rawAnswer : [rawAnswer];
    correctOptionIds = answerTexts
      .map((answer) => options.find((opt) => opt.text === normalizeAnswerText(answer))?.id)
      .filter(Boolean);
  }

  if (type === 'single') {
    if (options.length === 0) options.push(newOption('Option 1'), newOption('Option 2'));
...
export function normalizeQuiz(input, fallbackTitle = 'Imported Quiz') {
  const raw = Array.isArray(input) ? { questions: input, title: fallbackTitle } : input;
  if (!raw || !Array.isArray(raw.questions)) {
    throw new Error('The selected file is not a valid quiz JSON file.');
  }

  const questions = raw.questions.map(normalizeQuestion).filter((q) => q.content.trim() || q.imageDataUrl || q.options.length);
...
export function validateQuiz(quiz) {
  ...
  if (!Array.isArray(quiz.questions) || quiz.questions.length === 0) errors.push('Quiz must contain at least one question.');

  quiz.questions.forEach((q, idx) => {
    const label = `Question ${idx + 1}`;
    if (!q.content.trim() && !q.imageDataUrl) errors.push(`${label}: question content or image is required.`);
    if (q.type === 'text') {
      if (!q.correctText.trim()) errors.push(`${label}: text answer is required.`);
    } else {
      const texts = q.options.map((opt) => opt.text.trim()).filter(Boolean);
      if (texts.length < 2) errors.push(`${label}: at least two non-empty options are required.`);
      if (!q.correctOptionIds?.length) errors.push(`${label}: choose at least one correct answer.`);
      const validIds = new Set(q.options.map((opt) => opt.id));
      const invalid = q.correctOptionIds?.some((id) => !validIds.has(id));
      if (invalid) errors.push(`${label}: a correct answer points to a removed option.`);
      if (q.type === 'single' && q.correctOptionIds.length !== 1) errors.push(`${label}: single-choice questions need exactly one correct option.`);
    }
  });
...
export function prepareQuestions(quiz, settings) {
  let questions = quiz.questions.map((q) => ({
    ...q,
    options: settings.shuffleAnswers && q.type !== 'text' ? shuffleArray(q.options) : [...q.options],
  }));
...
export function correctAnswerText(question) {
  ...
  return question.options.filter((opt) => ids.has(opt.id)).map((opt) => opt.text).join(', ');
}

export function createResultPayload({ quiz, questions, answers, checks, settings, startedAt }) {
...
export function humanAnswer(question, answer) {
  ...
  if (question.type === 'single') return question.options.find((opt) => opt.id === answer)?.text || '';
